...
if (server=="it" && player == 188666){
	...
	if(localStorage.smartFakeSettings){
		var smartFakeSettings=localStorage.smartFakeSettings;  // <-- FIX: renomeado de 'settings' para 'smartFakeSettings'
		mode=smartFakeSettings.split(":::")[0];
		if (mode == "manual"){
			try{
			coords=JSON.parse(smartFakeSettings.split(":::")[1]);}
			catch(error){
					localStorage.removeItem("smartFakeSettings");
					UI.ErrorMessage("Script has been updated, you have to insert the settings again");
			}
			minArrival=new Date(smartFakeSettings.split(":::")[2]);
			maxArrival=new Date(smartFakeSettings.split(":::")[3]);
			troopPreference=JSON.parse(smartFakeSettings.split(":::")[4]);
		}
		else if(mode == "byUrl"){
			coords=getCoordsByUrl(smartFakeSettings.split(":::")[1]);
			dates=getArrivalDate(smartFakeSettings.split(":::")[2]);
			minArrival=dates[0];
			maxArrival=dates[1];
			troopPreference=JSON.parse(smartFakeSettings.split(":::")[3]);
		}
